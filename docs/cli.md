...
Хаб на хостинге с сигналом изменений (remote вида `https://login:token@host/git/<хаб>.git`,
хостинг отвечает на `GET /v1/hubs/<хаб>/wait?since=N&timeout=S`) `wait` не опрашивает по таймеру:
ждёт сигнал хостинга и забирает изменения только по нему, `--interval` задаёт лишь длину одного
ожидания. Нужен `curl`; без него, с другим remote или когда хостинг не отвечает — опрос по таймеру.
